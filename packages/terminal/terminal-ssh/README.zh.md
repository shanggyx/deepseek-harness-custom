...
每台启用的主机同时锚定一个远程工作区：`~/.dsh/remote-workspaces/<名称>` 下的一个真实本地目录，工作区注册表、沙箱与持久化都把它当作普通工作区对待。通过 `workspace.create { sshHost: <名称> }`（选择器的远程主机区由「连接」清单供给）创建工作区会备好锚点目录并注册；在其中创建的会话会收到一条 `ssh:workspace` 运行时上下文子句——与其他提示子句一样随运行时上下文快照落日志——指示 agent 通过 `ssh:<名称>` 终端在远程主机上工作，而不是使用本地文件工具。
