...
const row: SshHostRow = { name: 'box', host: 'h', port: 22, username: 'root', identityFile: '' }
...
describe('createSshHostsCardController', () => {
  ...
  it('appends a blank row and writes the whole roster on save', async () => {
    const scope = fakeScope({ status: 'ready', value: { hosts: [] }, writable: true })
    const { store, actions } = createSshHostsCardController(scope)

    actions.addRow()
    expect(store.getSnapshot().hosts).toEqual([
      { name: '', host: '', port: 22, username: 'root', identityFile: '' },
    ])
...
describe('decodeSshHostsDocument', () => {
  it('normalizes rows and fills a missing identity file', () => {
    expect(decodeSshHostsDocument({
      hosts: [{ name: 'box', host: 'h', port: 2222, username: 'root' }],
    })).toEqual({ hosts: [{ name: 'box', host: 'h', port: 2222, username: 'root', identityFile: '' }] })
  })
