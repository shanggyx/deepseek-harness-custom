...
### Shell surface: browser handoff, in-page title bar, and brand

The child runs with `--no-open`, so the web runtime's default-browser handoff stays off and the served page appears only inside the window; the 文件 menu's 在浏览器中打开 hands the original readiness URL (which carries the launch token) to the OS browser on demand. The window is chrome-less: `titleBarStyle: 'hidden'` plus an injected in-page strip replaces the native title bar and menu, because native chrome cannot follow the page's theme, cannot indent its labels, renders localized key names ("Ctrl+逗号"), and sizes its dropdowns itself. The strip is plain DOM over the page's theme tokens — it follows 浅色/深色/跟随浏览器 with no native sync — and models the editor-standard 文件/编辑/选择/查看/窗口/帮助 set with width-fitting dropdowns and right-aligned shortcut columns. Sidebar entries drive the served UI's own controls through CSS-module stem matching; main-process entries cross the preload bridge's one action channel. Every accelerator maps from the physical key in `before-input-event` (zoom from `=`/`+`/numpad, which Electron's zoomIn role misses) — the strip's shortcut texts are pure display. The product name is data, not code: the sidebar brand and the window title both fall back to the `brand.localBuild` locale strings, and the sidebar tests pin that copy.
